...
class ApkHttpServer(
    ...
    private val baseDir: File = File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "Gravity Installer")
    ...
    private fun serveDirectoryList(): Response {
        ...
        val listHtml = if (files.isEmpty()) {
            """<div class="empty"><div class="empty-icon">📭</div><div class="empty-text">No packages found.<br>Add files via Gravity Installer app.</div></div>"""
        } else {
            files.joinToString("\n") { file ->
